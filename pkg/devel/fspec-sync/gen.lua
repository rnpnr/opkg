cflags({
	...
	'-isystem $builddir/pkg/crypt/b3sum/include',
})

sub('host.ninja', function()
	set('outdir', '$outdir/host')
	toolchain(config.host)
	cflags({'-isystem $builddir/pkg/crypt/b3sum/include'})

	lib('libcommon.a', {'fatal.c', 'parse.c', 'reallocarray.c'})
	exe('fspec-hash', {'fspec-hash.c', 'libcommon.a', '$builddir/pkg/crypt/b3sum/host/libblake3.a'}, {'pkg/crypt/b3sum/headers'})
...
exe('fspec-hash', {'fspec-hash.c', 'libcommon.a', '$builddir/pkg/crypt/b3sum/libblake3.a'}, {'pkg/crypt/b3sum/headers'})
